...
const PharmacistsManagement = () => {
  ...
  const handleAddPharmacist = async () => {
    ...
    const passwordValidation = validatePasswordStrength(pharmacistForm.password);
                  ...
                  <div className="relative">
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setPharmacistForm({...pharmacistForm, password: newPassword});
                  ...
                  {pharmacistForm.password && (
                    <div className="mt-2 space-y-2">
                      ...
                      <div className="text-xs space-y-1 text-gray-600">
                        <div className={`flex items-center gap-1 ${pharmacistForm.password.length >= 8 ? 'text-green-600' : ''}`}>
                          {pharmacistForm.password.length >= 8 ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />}
                          <span>At least 8 characters</span>
                        </div>
                        <div className={`flex items-center gap-1 ${/[A-Z]/.test(pharmacistForm.password) ? 'text-green-600' : ''}`}>
                          {/[A-Z]/.test(pharmacistForm.password) ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />}
                        ...
                        <div className={`flex items-center gap-1 ${/[0-9]/.test(pharmacistForm.password) ? 'text-green-600' : ''}`}>
                          {/[0-9]/.test(pharmacistForm.password) ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />}
                          <span>One number (0-9)</span>
                        </div>
                        <div className={`flex items-center gap-1 ${/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(pharmacistForm.password) ? 'text-green-600' : ''}`}>
                          {/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(pharmacistForm.password) ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />}
