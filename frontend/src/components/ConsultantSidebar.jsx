...
export function ConsultantSidebar() {
  ...
  const getNavClassName = (path) => {
    const baseClasses = collapsed 
      ...
      : "w-full justify-start transition-all duration-200 ease-in-out rounded-lg mx-2 px-3 py-2.5 font-medium relative group";
  ...
  return (
    <Sidebar className={`border-r border-border bg-background dark:bg-background transition-all duration-300 ease-in-out shadow-sm h-screen sticky top-0 flex-shrink-0`} style={{ minHeight: '100vh', width: collapsed ? '4rem' : '16rem' }}>
      <SidebarContent className="h-full flex flex-col overflow-hidden" style={{ minHeight: '100vh' }}>
        {/* Header */}
        <div className="h-16 px-6 py-4 border-b border-border bg-background dark:bg-background flex items-center">
          <div className="flex items-center gap-3">
            ...
            {!collapsed && (
              <div>
                <h2 className="font-semibold text-foreground text-lg tracking-tight">Smaart Healthcare</h2>
              </div>
            )}
          </div>
        </div>
              ...
              {navigationItems.map((item) => (
                <SidebarMenuItem key={item.title}>
                  <Link to={item.url} className={getNavClassName(item.url)}>
                    <item.icon className={`w-5 h-5 transition-all duration-200 ${isActive(item.url) ? 'text-primary-foreground' : 'text-muted-foreground group-hover:text-accent-foreground'}`} />
                    {!collapsed && (
